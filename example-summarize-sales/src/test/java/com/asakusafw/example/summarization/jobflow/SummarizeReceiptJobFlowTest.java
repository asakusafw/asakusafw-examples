 ...
 */
public class SummarizeReceiptJobFlowTest {
	...
	JobFlowTester tester;

	@Before
	public void setup() {
		tester = new JobFlowTester(getClass());
	...
	@Test
	public void testSummarizeReceiptJobFlow() {

		tester.input("receipt", Receipt.class)
			.prepare(INPUT_PATH + "/receipt.xls#input");
		tester.input("shop", Shop.class)
			.prepare(INPUT_PATH + "/shop.xls#input");
		tester.input("item", Item.class)
			.prepare(INPUT_PATH + "/item.xls#input");

		tester.output("error", ErrorReceipt.class)
		...
		tester.output("itemSummary", ItemSummary.class)
		...
		tester.output("shopSummary", ShopSummary.class)
		...
		tester.runTest(SummarizeReceiptJobFlow.class);
	}
}
